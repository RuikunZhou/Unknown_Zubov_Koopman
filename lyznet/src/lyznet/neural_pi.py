...
def stage_cost_torch(x, u_func, Q_numpy, R_numpy):
    ...
    x_unsqueezed = x.unsqueeze(2).to(device)    
    x_cost = torch.bmm(
...
def get_controller_torch_from_Net(net, g_torch, R_inv_numpy):
    def u_func(x, net):
        ...
        R_inv_torch = R_inv_torch.to(device)
        g_torch_value = g_torch(x).transpose(1, 2).to(device)
...
def elm_pi(system, initial_u=None, num_of_iters=10, num_colloc_pts=3000,
    ...
    print("-" * 50)
    print(f"Total time for ELM training: {elapsed_time:.2f} seconds.")
...
def neural_pi(system, f_torch=None, g_torch=None, initial_u=None, 
    ...
    for iter_num in range(1, num_of_iters + 1):
        ...
        residual_computing = False
        if final_test==True and iter_num==num_of_iters:
            residual_computing = True
        ...
        net, model_path = lyznet.neural_learner(
            ...
            K=next_K_tensor, initial_net=initial_net, residual_computing=residual_computing
            )
    ...
    elapsed_time = end_time - start_time
    print("-" * 50)
    print(f"Total time for PINN training: {elapsed_time:.2f} seconds.")
