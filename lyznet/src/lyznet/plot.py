...
def plot_V(system, true_system=None, net=None, model_path=None, V_list=None, c_lists=None, 
           c1_V=None, c2_V=None, c1_P=None, c2_P=None, loss_mode="Zubov",
    ...
    if d == 1:
        ...
        if elm_model is not None: 
            weights, bias, beta = elm_model
            if loss_mode == "Zubov_Koopman":
                def elm_V(x):
                    H = np.matmul(x, weights.T) + bias.T
                    basis = np.concatenate((np.tanh(H), x.reshape(1,-1)), axis=1)
                    return basis @ beta
            else:
                def elm_V(x):
                    H = np.matmul(x, weights.T) + bias.T
        ...
        if c2_P is not None: 
            ax2.contour(x1, x2, quad_V_test, levels=[c2_P], 
                        colors='m', linewidths=2, linestyles='--')
        ...
        if phase_portrait is not None:
            plot_phase_portrait(x1, x2, true_system,
                                closed_loop_f_numpy=closed_loop_f_numpy)

        if c2_V is not None:
            ax2.clabel(cs, inline=1, fontsize=10)

        # # add a circle of radius 1.2 centered at the origin
        # circle = Circle((0, 0), 1.1, color='r', fill=False, linestyle='-.')
        # ax2.add_patch(circle)
        ax2.set_xlabel(r'$x_1$', fontsize=24)
        ax2.set_ylabel(r'$x_2$', fontsize=24)
        ax2.set_title('Level sets')
        # set the legend loc to upper left
        # ax2.legend(loc='upper left')
        handles = [
        plt.Line2D([], [], color='b', linewidth=3, label='Koopman-based'),
        plt.Line2D([], [], color='m', linestyle='--', linewidth=2, label='Quadratic')
        ]

        ax2.legend(handles=handles, loc='lower right', frameon=True, facecolor='white', framealpha=1.)
...
def plot_V_Koopman(system, true_system=None, net=None, model_path=None, V_list=None, c_lists=None, 
           c1_V=None, c2_V=None, c1_P=None, c2_P=None, loss_mode="Zubov",
           phase_portrait=None, elm_model=None, poly_model=None, lie_derivative=None, 
           plot_trajectories=None, n_trajectories=50, plot_cost=None,
           u_func=None, Q=None, R=None, closed_loop_f_numpy=None,
           obstacles=None):
    domain = system.domain
    ...
    if d == 1:
        ...
        if poly_model is not None:
            m_monomial, n_monomial, beta = poly_model
            basis_values = np.array([input_array[:,0]**m *input_array[:,1]**n
                                    for n in range(n_monomial)
                                    for m in range(m_monomial)])
            V_test = basis_values.T @ beta
            # reshape V_test for plotting
            V_test = V_test.reshape(x1.shape)
        ...
        # # add a circle of radius 1.2 centered at the origin
        # circle = Circle((0, 0), 1.2, color='r', fill=False, linestyle='-.')
        # ax2.add_patch(circle)
        ax2.set_xlabel(r'$x_1$', fontsize=24)
        ax2.set_ylabel(r'$x_2$', fontsize=24)
        ax2.set_title('Level sets')
        ...
        plt.Line2D([], [], color='b', linewidth=3, label='Koopman-based'),
        plt.Line2D([], [], color='m', linestyle='--', linewidth=2, label='Quadratic'),
        plt.Line2D([], [], color='g', linestyle='-.', linewidth=2, label='NN-based')
        ]

        ax2.legend(handles=handles, loc='upper left', frameon=True, facecolor='white', framealpha=0.5)
