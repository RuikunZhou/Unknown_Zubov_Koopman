...
def numpy_poly2_verifier(system, m_monomial, n_monomial, beta, c2_P, epsilon=1e-4, loss_mode="Lyapunov",
    ...
    for n in range(n_monomial):
        for m in range(m_monomial):
            ...
            basis_sym.append((x[0]**m) * (x[1]**n))

    V_learn = np.dot(basis_sym, beta.T)
    print("V = ", V_learn)
...
def numpy_elm_verifier(system, weights, bias, beta, c2_P, epsilon=1e-4, loss_mode="Lyapunov",
                       tol=1e-4, accuracy=1e-2, number_of_jobs=32):
    config = lyznet.utils.config_dReal(number_of_jobs=number_of_jobs, tol=tol)
    xlim = system.domain
