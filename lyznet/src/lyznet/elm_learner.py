...
def numpy_elm_learner(system, num_hidden_units=100, num_colloc_pts=3000, 
    ...
    def compute_residuals_vectorized(samples, weights, bias, omega=omega, 
        ...
        if loss_mode == "Zubov_Koopman": 
            identity_gradient = np.tile(np.eye(samples.shape[1]), (1, batched_gradients.shape[1], 1))
            identity_gradient = identity_gradient.reshape(samples.shape[1], batched_gradients.shape[1], samples.shape[1])
            Koopman_gradients = np.concatenate((batched_gradients, identity_gradient), axis=0)
            Koopman_basis = np.concatenate((sigma_H, samples), axis=1)
            A = (np.einsum('mnd,nd->mn', Koopman_gradients, f_x) 
                 - mu * np.einsum('nm,n->mn', Koopman_basis, omega_values))
            b = - mu * omega_values
        ...
        # elif loss_mode == "Lyapunov_Koopman":
        #     A = np.einsum('mnd,nd->mn', batched_gradients, f_x)
        #     b = - Q(samples)
        else:
            A = np.einsum('mnd,nd->mn', batched_gradients, f_x)
    ...
    def compute_zero_boundary_Koopman(samples, weights, bias):
        N = samples.shape[0]
        m = weights.shape[0] + samples.shape[1]
        A = np.zeros((m, N))
        b = np.zeros(N)    
        for i in range(N):
            x = samples[i].reshape(-1, 1)
            H = np.matmul(weights, x) + bias
            sigma_H = activation(H)
            sigma_H = np.vstack((sigma_H, samples.T))
            A[:, i] = sigma_H.squeeze() 
        return A, b  
    ...
    def compute_one_boundary_Koopman(samples, weights, bias):
        H = np.dot(weights, samples.T) + bias
        A = activation(H)
        # concatenate samples to A
        A = np.concatenate((A, samples.T), axis=0)
        # print('A: ', A.shape)
        N = samples.shape[0]
        b = np.ones(N) 
        return A, b    
    ...
    if loss_mode == "Zubov_Koopman":
        A2, b2 = compute_zero_boundary_Koopman(x0, weights, bias)
    else:
    ...
    if (((loss_mode == "Zubov" or loss_mode == "Zubov_Koopman" or loss_mode == "Zubov_GHJB") and data is None)
            and one_boundary):
        # Add extra 1-boundary condition for solving Zubov equation
        boundary_points = generate_boundary_points(system.domain, 100)
        if loss_mode == "Zubov_Koopman":
            A3, b3 = compute_one_boundary_Koopman(boundary_points, weights, bias)
        else:
    ...
    if residuals.size == 0:
        ...
        residual = b_reg - np.dot(A_reg, beta)
        residuals = np.sum(residual ** 2)
    else:
        residuals = residuals

    print("Square Residual Sum:", residuals)  
    print("Mean square error:", residuals/len(beta))  
    if test is True:
