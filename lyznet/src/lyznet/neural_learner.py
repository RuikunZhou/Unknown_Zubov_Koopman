...
def Lyapunov_GHJB_loss(x, net, system, omega=None, u_func=None, 
                       f_torch=None, g_torch=None, R_inv=None, K=None, final_test=False):
    ...
    if g_torch is None:
        ...
        # print("-"*50)
    g_values = g_torch(x).to(device)
    # print("g:", g_values.shape)
    u_values = u_func(x).to(device)
    ...
    g_values_zero = g_torch(zero_tensor).to(device)
    # print("g: ", g_values_zero.shape)
    g_V = torch.bmm(g_values_zero.transpose(1, 2), 
                    DV_zero.unsqueeze(0).transpose(1, 2)).squeeze()

    R_inv_tensor = torch.tensor(R_inv, dtype=torch.float32).unsqueeze(0).to(device)
...
def loss_function_selector(loss_mode, net, x, system, data_tensor, 
                           ...
                           R_inv, K, V_clf,final_test=False):
    if loss_mode == 'Zubov':
...
def neural_learner(system, data=None, loss_mode='Zubov', layer=3, width=10, 
                   num_colloc_pts=100000, batch_size=512, max_epoch=5, 
                   ...
                   initial_net=None, homo_deg=None, V_clf=None, residual_computing=None): 
    ...
    if residual_computing: 
        print("Testing residual errors...")
        lyznet.tik() 
        x_test = torch.rand((num_colloc_pts*2, d)).to(device)
        x_test = x_test * (ranges[:, 1] - ranges[:, 0]) + ranges[:, 0]

        loss = loss_function_selector(loss_mode, net, x_test, 
                                          system, data_tensor, 
                                          v_max, transform, omega, u_func,
                                          f_torch, g_torch, R_inv, K, V_clf, final_test=True)
        print("Maximum Test Residual Error:", torch.max(loss))
        lyznet.tok()   
